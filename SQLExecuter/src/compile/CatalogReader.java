...
public class CatalogReader {
  ...
  private Map <String, CountedTableData> returnVal = null;
  ...
  public static String printCatalog (Map <String, CountedTableData> printMe) {
  
    String retVal = "{\n";
    for (Map.Entry<String,CountedTableData> i : printMe.entrySet ()) {
      retVal += "\n(" + i.getKey () + ",\n{" + i.getValue ().print () + ")";
    }
    retVal += "}";
    return retVal;
  ...
  public Map <String, CountedTableData> getCatalog () {
    if (returnVal == null) {
      returnVal = new HashMap <String, CountedTableData> ();
    }
    for (int i = 0; i < numTables; i++) {
      ...
      Map <String, CountedAttInfo> attributes = new HashMap <String, CountedAttInfo> ();
      for (int j = 0; j < attCount; j++) {
        ...
        suckToTag ("</att>");
        attributes.put (attName, new CountedAttInfo (attName,numVals, myType, j));
      }
      returnVal.put (tableName, new CountedTableData (tableName,tupleCount, attributes));
      suckToTag ("</table>");
    }
    suckToTag ("</catalog>");
    return returnVal;
